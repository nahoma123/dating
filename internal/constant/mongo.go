...
Filters
 ...
 4. not equals [!=]
*/
func GetResults(cxt context.Context, db *mongo.Database, collectionName string, filterPagination *FilterPagination, results *[]bson.M, geoFilter bson.M, additionalFilters ...bson.M) error {
	...
	for _, fl := range additionalFilters {
		pipeline = append(pipeline, fl)
	}
...
Filters
 ...
 4. not equals [!=]
*/
func GetRecommendationsDb(cxt context.Context, db *mongo.Database, collectionName string, filterPagination *FilterPagination, results *[]bson.M, geoFilter bson.M, removeProfileIds []string) error {
	// Get a reference to the collection
	collection := db.Collection(collectionName)
	...
	if len(removeProfileIds) != 0 {
		for _, id := range removeProfileIds {
			filters = append(filters, bson.M{"profile_id": bson.M{"$ne": id}})
		}
	}
	// Create pipeline with filter stage if there are filters
	var pipeline []bson.M
